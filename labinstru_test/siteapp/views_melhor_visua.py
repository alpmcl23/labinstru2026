...
# -*- coding: utf-8 -*-
"""
Painel INMET — view principal e cliente de dados.

- Busca dados no INMET
- Converte datas para America/Manaus
- Gera resumos e HTML dos gráficos (Plotly) para:
  • Tempo real (gauges de Temperatura, Umidade, Pressão)
  • Resumo do dia
...
def get_station_data(
    ...
    """
    Busca dados no INMET e retorna DataFrame com índice (timezone-aware) em America/Manaus.
    Nunca propaga exceções: retorna (df|None, erro|None).
    """
    url = (
        f"https://apitempo.inmet.gov.br/token/estacao/"
        f"{start_date}/{end_date}/{station}/{INMET_TOKEN}"
    )
    ...
    try:
        ...
        expected = ["TEM_INS", "UMD_INS", "PRE_INS", "VEN_VEL", "CHUVA", "PTO_INS"]
        for col in expected:
            if col not in df.columns:
                df[col] = pd.NA
            df[col] = pd.to_numeric(df[col], errors="coerce")

        # Datetime: string -> UTC -> America/Manaus
        dt = pd.to_datetime(
            df["DT_MEDICAO"].astype(str)
            + " "
            + df["HR_MEDICAO"].astype(str).str.zfill(4),
            format="%Y-%m-%d %H%M",
            errors="coerce",
        )
        ok = dt.notna()
    ...
    except Exception as e:  # capturamos tudo e retornamos como mensagem
...
def inmet_painel(request, station: str = "A101"):
    """
    Painel com:
      - Tempo real: pílula + gauges (Temperatura, Umidade, Pressão)
      - Resumo do dia + gráfico
      - Semana (últimos 7 dias)
      - Mensal (1º → último dia)
    Mantém comportamento “fail-soft”: sempre entrega HTML válido.
    """
    cache_key = f"inmet_panel_v1_gauges_{station}"
    ...
    if ctx is None:
        ...
        # --- Janelas de busca ---
        week_start_api = (now - timedelta(days=8)).strftime("%Y-%m-%d")  # margem extra
        ...
        month_end_date = datetime(
            now.year, now.month, month_last_day, tzinfo=TZ_AM
        ).date()

        # --- Busca de dados ---
        df_week, err_w = get_station_data(station, week_start_api, today_api)
        df_month_all, err_m = get_station_data(
            station, month_start_date.strftime("%Y-%m-%d"), today_api
        )
        api_error = err_w or err_m  # apenas informativo
        ...
        if isinstance(df_week, pd.DataFrame) and not df_week.empty:
            ...
            if not valid.empty:
                ...
                temp_current = (
                    f"{last.get('TEM_INS'):.1f}" if pd.notna(last.get("TEM_INS")) else "-"
                )
                umid_current = (
                    f"{last.get('UMD_INS'):.1f}" if pd.notna(last.get("UMD_INS")) else "-"
                )
                chuva_current = (
                    f"{last.get('CHUVA'):.1f}" if pd.notna(last.get("CHUVA")) else "-"
                )

        # ========= GAUGES Tempo real =========
        graph_temp_gauge = '<div class="alert alert-info m-0">Sem temperatura recente.</div>'
        graph_humi_gauge = '<div class="alert alert-info m-0">Sem umidade recente.</div>'
        graph_pres_gauge = '<div class="alert alert-info m-0">Sem pressão recente.</div>'

        try:
            if isinstance(df_week, pd.DataFrame) and not df_week.empty:
                last_valid = df_week.iloc[-1]

                # Valores numéricos (com fallback)
                t_val = float(last_valid.get("TEM_INS")) if pd.notna(last_valid.get("TEM_INS")) else None
                h_val = float(last_valid.get("UMD_INS")) if pd.notna(last_valid.get("UMD_INS")) else None
                p_val = float(last_valid.get("PRE_INS")) if pd.notna(last_valid.get("PRE_INS")) else None  # hPa

                # --- Temperatura (gauge semicircular) ---
                if t_val is not None:
                    fig_t = go.Figure(go.Indicator(
                        mode="gauge+number",
                        value=t_val,
                        number={'suffix': " °C", 'font': {'size': 28}},
                        title={'text': "Temperatura", 'font': {'size': 14}},
                        gauge={
                            'axis': {'range': [0, 45], 'tickwidth': 1, 'tickcolor': "#95a5a6"},
                            'bar': {'color': '#333', 'thickness': 0.28},
                            'borderwidth': 0,
                            'steps': [
                                {'range': [0, 30], 'color': '#137a44'},     # verde
                                {'range': [30, 35], 'color': '#f6c745'},    # amarelo
                                {'range': [35, 40], 'color': '#f28e2b'},    # laranja
                                {'range': [40, 45], 'color': '#e15759'},    # vermelho
                            ]
                        },
                        domain={'x': [0, 1], 'y': [0, 1]}
                    ))
                    fig_t.update_layout(margin=dict(l=10, r=10, t=30, b=10), height=220, template="plotly_white")
                    graph_temp_gauge = fig_t.to_html(full_html=False, include_plotlyjs=False)
                ...
                if h_val is not None:
                    fig_h = go.Figure(go.Indicator(
                        ...
                        title={'text': "Umidade", 'font': {'size': 14}},
                        gauge={
                            'axis': {'range': [0, 100], 'tickwidth': 0, 'ticks': ''},
                            'bar': {'color': '#2b6cb0', 'thickness': 0.25},
                            'bgcolor': '#e8f2fe',
                            'borderwidth': 0
                        },
                        domain={'x': [0, 1], 'y': [0, 1]}
                    ))
                    fig_h.update_layout(margin=dict(l=10, r=10, t=30, b=10), height=220, template="plotly_white")
                    graph_humi_gauge = fig_h.to_html(full_html=False, include_plotlyjs=False)

                # --- Pressão (gauge semicircular) ---
                if p_val is not None:
                    # INMET em hPa; faixa típica 980–1030 hPa
                    p_min, p_max = 980, 1030
                    fig_p = go.Figure(go.Indicator(
                        mode="gauge+number",
                        value=p_val,
                        number={'suffix': " hPa", 'font': {'size': 28}},
                        title={'text': "Pressão", 'font': {'size': 14}},
                        gauge={
                            'axis': {'range': [p_min, p_max], 'tickwidth': 1, 'tickcolor': "#95a5a6"},
                            'bar': {'color': '#333', 'thickness': 0.28},
                            'borderwidth': 0,
                            'steps': [
                                {'range': [p_min, 1000], 'color': '#e15759'},   # baixo (vermelho)
                                {'range': [1000, 1016], 'color': '#f6c745'},    # normal (amarelo)
                                {'range': [1016, p_max], 'color': '#137a44'},   # alto (verde)
                            ]
                        },
                        domain={'x': [0, 1], 'y': [0, 1]}
                    ))
                    fig_p.update_layout(margin=dict(l=10, r=10, t=30, b=10), height=220, template="plotly_white")
                    graph_pres_gauge = fig_p.to_html(full_html=False, include_plotlyjs=False)
        except Exception as e:
            _dbg(f"ERRO gauges: {e}")
        ...
        try:
            ...
            if base is not None and not base.empty:
                ...
                if not df_today.empty:
                    ...
                    ch_sum = df_today["CHUVA"].sum(min_count=1)
                    if pd.notna(tmin) and pd.notna(tmax):
                        pill_diario = (
                            f"Mín {tmin:.1f} °C · Máx {tmax:.1f} °C · "
                            f"Chuva {float(ch_sum or 0):.1f} mm"
                        )

                    # Gráfico (chuva, umidade, temperatura e “sensação”)
                    fig = go.Figure()
                    fig.add_trace(
                        go.Bar(
                            x=df_today.index,
                            y=df_today["CHUVA"].fillna(0),
                            name="Precipitação",
                            opacity=0.55,
                            marker_line_width=0,
                            yaxis="y3",
                    ...
                    fig.update_layout(
                        ...
                        xaxis=dict(
                            title="Horas (America/Manaus)", tickformat="%H:%M", showgrid=True
                        ),
        ...
        # ===================== SEMANA (ÚLTIMOS 7 DIAS) =====================
        graph_temp_week = (
            '<div class="alert alert-info m-0">Sem dados para os últimos 7 dias.</div>'
        )
        graph_chuva_week = (
            '<div class="alert alert-info m-0">Sem dados para os últimos 7 dias.</div>'
        )
        ...
        try:
            ...
            idx7 = pd.date_range(
                start=pd.Timestamp(start7, tz=TZ_AM),
                end=pd.Timestamp(end7, tz=TZ_AM),
                freq="D",
            )

            if isinstance(df_week, pd.DataFrame) and not df_week.empty:
                w = df_week[
                    (df_week.index.date >= start7) & (df_week.index.date <= end7)
                ].copy()

                weekly = (
                    w.resample("D")
                    .agg({"TEM_INS": ["mean", "max"], "CHUVA": "sum"})
                    .reindex(idx7)
                )
                ...
                total_chuva_week = float(
                    weekly.get("CHUVA_sum", pd.Series(dtype=float)).sum() or 0.0
                )
                mean_of_max = float(
                    weekly.get("TEM_INS_max", pd.Series(dtype=float)).mean()
                    or float("nan")
                )

                pill_semana = (
                    f"Média da Máx. {mean_of_max:.1f} °C · "
                    f"Chuva total {total_chuva_week:.1f} mm"
                )

                fig_wt = px.line(
                    weekly, y="TEM_INS_mean", title="Temperatura Média — últimos 7 dias (°C)"
                )
        ...
        try:
            idx_month = pd.date_range(
                start=pd.Timestamp(month_start_date, tz=TZ_AM),
                end=pd.Timestamp(month_end_date, tz=TZ_AM),
                freq="D",
            )

            if isinstance(df_month_all, pd.DataFrame) and not df_month_all.empty:
                m_metrics = df_month_all[
                    (df_month_all.index.date >= month_start_date)
                ...
                if not monthly.empty:
                    ...
                    pill_mensal = (
                        f"Média {tmean:.1f} °C · "
                        f"Máx {tmax:.1f} °C ({_d(tmax_day)}) · "
                        f"Mín {tmin:.1f} °C ({_d(tmin_day)}) · "
                        f"Chuva {rain_total:.1f} mm"
                    )
                ...
                fig_m.update_layout(
                    ...
                    xaxis=dict(
                        ...
                        showgrid=True,
                        range=[
                            pd.Timestamp(month_start_date, tz=TZ_AM),
                            pd.Timestamp(month_end_date, tz=TZ_AM),
                        ],
                    ),
                    yaxis=dict(title="Temperatura (°C)", showgrid=True),
                    yaxis2=dict(
                        title="Chuva (mm)", overlaying="y", side="right", showgrid=False
                    ),
                    legend=dict(
                        orientation="h",
                        yanchor="bottom",
        ...
        ctx = {
            ...
            "umid_current": umid_current,
            "chuva_current": chuva_current,

            # gauges tempo real
            "graph_temp_gauge": graph_temp_gauge,
            "graph_humi_gauge": graph_humi_gauge,
            "graph_pres_gauge": graph_pres_gauge,

            "pill_diario": pill_diario,
            "pill_semana": pill_semana,
            "pill_mensal": pill_mensal,
            "pill_anual": "—",
            "pill_clima": "—",
            ...
            "error_message": api_error,  # opcional exibir
        }
