...
# siteapp/views.py (somente a função inmet_painel e helpers usados por ela)
...
STATIONS = {
    "A101": {"name": "Manaus (Eduardo Gomes)", "lat": -3.118, "lon": -60.021},
    "A116": {"name": "Manaus (Ponta Pelada)",   "lat": -3.146, "lon": -59.986},
}

STATE_PREFIX = {
    "Amazonas": "13", "Roraima": "14", "Rondônia": "11",
    "Acre": "12", "Pará": "15", "Mato Grosso": "51",
}
...
def _fmt_date(iso):
    ...
    for f in ("%Y-%m-%dT%H:%M:%S.%fZ", "%Y-%m-%dT%H:%M:%SZ"):
        try:
            dt = datetime.strptime(iso, f)
            return dt.strftime("%d/%m/%Y"), dt.strftime("%Hh%Mmin")
        except Exception:
            pass
    return iso, ""

def _serie_semana_base(now):
    dias = [now - timedelta(days=6 - i) for i in range(7)]
    x = [d.strftime("%d/%m") for d in dias]
    tmax = [35.0, 35.4, 33.9, 33.7, 32.1, 31.0, 30.0]
    tmin = [25.7, 26.9, 26.4, 26.6, 26.1, 22.8, 25.6]
    rain = [0.0, 0.0, 0.8, 0.0, 2.5, 35.0, 0.0]
    return x, tmax, tmin, rain

def _grafico_semana(x, tmax, tmin, rain):
    if not go:
        return "<div style='padding:16px'>Instale <code>plotly</code> para ver o gráfico.</div>"
    fig = go.Figure()
    fig.add_bar(name="Chuva (mm)", x=x, y=rain, opacity=0.45, marker_line_width=0, yaxis="y2")
    fig.add_scatter(name="Temp máx (°C)", x=x, y=tmax, mode="lines+markers")
    fig.add_scatter(name="Temp mín (°C)", x=x, y=tmin, mode="lines+markers")
    fig.update_layout(
        title="Semana — Temperaturas e Chuva", margin=dict(l=30,r=30,t=40,b=30),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        xaxis_title="Dia (Manaus)",
        yaxis=dict(title="Temperatura (°C)", rangemode="tozero"),
        yaxis2=dict(title="Chuva (mm)", overlaying="y", side="right", rangemode="tozero"),
        template="plotly_white", hovermode="x unified",
    )
    return fig.to_html(full_html=False, include_plotlyjs="cdn", config={"displayModeBar": False})

def _fetch_alert_geo(estados_sel):
    """Busca INMET e devolve GeoJSON + um aviso inicial com campos formatados."""
    ...
    try:
        ...
        return {"geojson_str": "{}", "selected": None, "error": f"Falha ao consultar INMET: {e}"}

    # filtra por estados (hoje + futuro)
    avisos = []
    for k in ("hoje", "futuro"):
        for av in data.get(k, []):
            if any(e in (av.get("estados") or []) for e in estados_sel):
                avisos.append(av)

    if not avisos:
        return {"geojson_str": "{}", "selected": None, "error": "Sem alertas para os estados selecionados."}

    prefixes = {STATE_PREFIX[e] for e in estados_sel}
    features, selected = [], None

    for i, av in enumerate(avisos):
        # formatar datas
        dinicio_d, dinicio_h = _fmt_date(av.get("data_inicio"))
        dfim_d, dfim_h       = _fmt_date(av.get("data_fim"))

        # riscos e instruções
        riscos = av.get("riscos", [])
        if isinstance(riscos, list): riscos = ". ".join([str(x) for x in riscos]).strip(". ")
        instr = av.get("instrucoes", [])
        if isinstance(instr, list): instr = ". ".join([str(x) for x in instr]).strip(". ")

        # municípios
        if av.get("municipios"):
            if isinstance(av["municipios"], list):
                municipios_view = ", ".join(m for m in av["municipios"] if m)
            else:
                municipios_view = str(av["municipios"])
        else:
            raw = av.get("geocodes", "") or ""
            codes = [c.strip() for c in raw.split(",") if c.strip()]
            codes = [c for c in codes if any(c.startswith(p) for p in prefixes)]
            municipios_view = ", ".join(codes) if codes else "—"

        props = {
            **av,
            "data_inicio_formatado": dinicio_d, "hora_inicio": dinicio_h,
            "data_fim_formatado": dfim_d,       "hora_fim": dfim_h,
            "riscos_txt": riscos,
            "instr_txt": instr,
            "municipios_view": municipios_view,
            "areas_afetadas": av.get("areas_afetadas", av.get("areas", "—")),
            "aviso_cor": av.get("aviso_cor", "#ffd300"),
        }

        # polígono
        try:
            geom = json.loads(props["poligono"])
            features.append({"type": "Feature", "geometry": geom, "properties": props})
            if selected is None:
                selected = props
        except Exception:
            # sem polígono: painel funciona mesmo assim
            if selected is None:
                selected = props

    geojson = {"type": "FeatureCollection", "features": features}
    return {"geojson_str": json.dumps(geojson, ensure_ascii=False), "selected": selected, "error": ""}

def inmet_painel(request, station):
    """Renderiza seu inmet_painel.html com a nova aba ALERTA no estilo ALERT-AS."""
    active_tab = request.GET.get("tab", "tempo")
    now = timezone.localtime()
    st = STATIONS.get(station, {"name": station, "lat": -3.118, "lon": -60.021})

    # ======== abas de estação (mesmo de antes) ========
    xs, tmax, tmin, rain = _serie_semana_base(now)
    graph_week_combo = _grafico_semana(xs, tmax, tmin, rain) if go else "<div></div>"
    pill_diario = f"{now.strftime('%d/%m')} • últimas 24h"
    graph_daily = graph_week_combo  # só pra não deixar vazio; troque por sua função
    graph_month = graph_week_combo

    week_tmax = round(max(tmax), 1); week_tmin = round(min(tmin), 1)
    week_tmax_day = xs[tmax.index(max(tmax))]; week_tmin_day = xs[tmin.index(min(tmin))]
    week_temp_avg = round(sum((a+b)/2 for a,b in zip(tmax, tmin))/len(tmax), 1)
    week_rain_total = round(sum(rain), 1); week_rain_days = sum(1 for r in rain if r>0); week_dry_days = len(rain)-week_rain_days

    # ======== ALERTA estilo ALERT-AS ========
    estados_sel = ["Amazonas", "Roraima", "Rondônia", "Acre", "Pará", "Mato Grosso"]
    alert_geo = _fetch_alert_geo(estados_sel)

    ctx = {
        "station": station, "station_name": st["name"], "station_lat": st["lat"], "station_lon": st["lon"],
        "active_tab": active_tab, "update_date": now.strftime("%d/%m/%Y"), "update_time": now.strftime("%H:%M"),

        # tempo real
        "rt_temp_value": 29.7, "rt_feels_value": 33.1, "rt_humi_value": 78, "rt_pres_value": 1008, "rt_wind_value": 2.3,

        # diário
        "pill_diario": pill_diario, "graph_daily": graph_daily,

        # semanal
        "week_tmax": week_tmax, "week_tmin": week_tmin, "week_tmax_day": week_tmax_day, "week_tmin_day": week_tmin_day,
        "week_temp_avg": week_temp_avg, "week_rain_total": week_rain_total, "week_rain_days": week_rain_days,
        "week_dry_days": week_dry_days, "graph_week_combo": graph_week_combo,

        # mensal
        "month_label": now.strftime("%B/%Y").capitalize(), "month_tmax": week_tmax, "month_tmin": week_tmin,
        "month_rain_total": week_rain_total, "month_rain_days": week_rain_days, "month_dry_days": week_dry_days,
        "graph_month": graph_month,

        # alerta (mapa+painel)
        "geojson_str": alert_geo["geojson_str"],
        "alert_selected": alert_geo["selected"],   # dict com campos já formatados
        "alert_error": alert_geo["error"],
    }
